<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <meta http-equiv="x-ua-compatible" content="ie=edge">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>changelog /// memex</title>
  <?php include('includes/head.php'); ?>
</head>
<body>
  <!-- CONTENT GOES HERE -->
  <header>
    <?php include('includes/header-nav.php'); ?>
  </header>
  <div class="col-1 content"><h1><span>Changelog</span></h1></div>

  
  <article class="col-2">
    <div class="column content prose">
    <p><strong>2024-SEP-01</strong> : Updated nav design; added aspect ratios</p>
    <p><strong>2024-JUN-29</strong> : New page: <a href="all.php">all</a></p>
    <p><strong>2024-MAY-19</strong> : Added images :
        <a href="inbox.php">inbox,</a>
        <a href="w-agency-portfolio.php">agency × portfolio,</a>
        <a href="architecture-space.php">architecture × space,</a>
        <a href="art-illustration.php">art × illustration,</a>
        <a href="d-brand-logos.php">brand × logos,</a>
        <a href="color.php">color,</a>
        <a href="w-elements.php">elements,</a>
        <a href="ephemera.php">ephemera,</a>
        <a href="d-process.php">process,</a>
        <a href="strategy.php">strategy,</a>
        <a href="d-typography.php">typography,</a>
        <a href="w-ux-wireframes.php">ux × wireframe,</a>
        <a href="d-visual-design.php">visual design,</a>
        <a href="w-websites.php">websites</a>
    </p>

    <p><strong>2024-MAR-03</strong> Added images : 
        <a href="inbox.php">inbox,</a>
        <a href="w-agency-portfolio.php">agency × portfolio,</a>
        <a href="d-apps.php">apps,</a>
        <a href="d-brand-logos.php">brand × logos,</a>
        <a href="d-data-visualization.php">data visualization,</a>
        <a href="goods.php">goods,</a>
        <a href="quotes.php">quotes,</a>
        <a href="w-websites.php">websites;</a>

        Created new index pages for <a href="design.php">design</a> and <a href="websites.php">websites</a>; moved <a href="goods.php">goods</a> to its own directory; updated renderer to display the number of items in each category
      </p>

      <p><strong>2023-NOV-12</strong> : Added images :
          <a href="inbox.php">inbox,</a>
          <a href="architecture-space.php">architecture × space,</a>
          <a href="art-illustration.php">art × illustration,</a>
          <a href="d-brand-logos.php">brand × logos,</a>
          <a href="d-business-cards.php">business cards,</a>
          <a href="d-typography.php">typography,</a>
          <a href="d-visual-design.php">visual design</a>
      </p>
      <p>
        <strong>2023-JUL-22</strong> : Added images :
        <a href="inbox.php">inbox</a>
      </p>
      <p><strong>2023-APR-25</strong> : Added images :
          <a href="inbox.php">inbox,</a>
          <a href="w-agency-portfolio.php">agency × portfolio,</a>
          <a href="art-illustration.php">art × illustration,</a>
          <a href="d-brand-logos.php">brand × logos,</a>
          <a href="color.php">color,</a>
          <a href="copywriting.php">copywriting,</a>
          <a href="d-data-visualization.php">data visualization,</a>
          <a href="w-elements.php">elements,</a>
          <a href="w-product.php">product,</a>
          <a href="quotes.php">quotes,</a>
          <a href="d-typography.php">typography,</a>
          <a href="w-ux-wireframes.php">ux × wireframe,</a>
          <a href="d-visual-design.php">visual design,</a>
          <a href="w-websites.php">websites</a>
      </p>
      <p>
        <strong>2023-MAR-15</strong> : Added images :
        <a href="inbox.php">inbox</a>
      </p>
      <p>
        <strong>2023-JAN-15</strong> : Moved 'design ― space' to <a href="architecture-space.php">architecture × space</a>
      </p>
      <p>
        <strong>2023-JAN-03</strong> : Added images :
        <a href="inbox.php">inbox</a>
      </p>
      <p><strong>2022-DEC-03</strong> : Added images :
        <a href="inbox.php">inbox</a>
      </p>
      <p><strong>2022-OCT-22</strong> : Added images :
          <a href="inbox.php">inbox</a>
      </p>
      <p><strong>2022-OCT-15</strong> : Added images :
          <a href="inbox.php">inbox</a>; converted all images to webp; updated renderer to support webp
      </p>
      <p><strong>2022-SEP-18</strong> : Added images :
          <a href="inbox.php">inbox,</a>
          <a href="w-agency-portfolio.php">agency × portfolio,</a>
          <a href="d-apps.php">apps,</a>
          <a href="art-illustration.php">art × illustration,</a>
          <a href="d-business-cards.php">business cards,</a>
          <a href="color.php">color,</a>
          <a href="copywriting.php">copywriting,</a>
          <a href="d-data-visualization.php">data visualization,</a>
          <a href="w-elements.php">elements,</a>
          <a href="ephemera.php">ephemera,</a>
          <a href="goods.php">goods,</a>
          <a href="d-patterns.php">patterns,</a>
          <a href="photography.php">photography,</a>
          <a href="w-product.php">product,</a>
          <a href="d-space.php">space,</a>
          <a href="d-typography.php">typography,</a>
          <a href="w-ux-wireframes.php">ux × wireframes,</a>
          <a href="d-visual-design.php">visual design,</a>
          <a href="w-websites.php">websites</a>
      </p>

      <p><strong>2022-AUG-10</strong> : Added images :
          <a href="inbox.php">inbox</a>
      </p>

      <p><strong>2022-JUL-09</strong> : Added images :
          <a href="inbox.php">inbox</a>
      </p>

      <p><strong>2022-APR-24</strong> : Added images :
          <a href="inbox.php">inbox</a>
      </p>

      <p><strong>2022-APR-16</strong> : Added images :
        <a href="w-agency-portfolio.php">agency × portfolio,</a>
        <a href="d-apps.php">apps,</a>
        <a href="art-illustration.php">art × illustration,</a>
        <a href="d-business-cards.php">business cards,</a>
        <a href="ephemera.php">ephemera,</a>
        <a href="goods.php">goods,</a>
        <a href="photography.php">photography,</a>
        <a href="d-space.php">space,</a>
        <a href="strategy.php">strategy,</a>
        <a href="d-typography.php">typography,</a>
        <a href="w-ux-wireframes.php">ux × wireframes,</a>
        <a href="d-visual-design.php">visual design,</a>
        <a href="w-websites.php">websites,</a>
        <a href="w-elements.php">website elements</a>
      </p>

      <p><strong>2022-JAN-09</strong> : Added images :
        <a href="d-apps.php">apps,</a>
        <a href="w-agency-portfolio.php">agency × portfolio,</a>
        <a href="art-illustration.php">art × illustration,</a>
        <a href="d-brand-logos.php">brand × logos,</a>
        <a href="color.php">color,</a>
        <a href="w-elements.php">elements,</a>
        <a href="ephemera.php">ephemera,</a>
        <a href="d-business-cards.php">business cards,</a>
        <a href="d-data-visualization.php">data visualization,</a>
        <a href="goods.php">goods,</a>
        <a href="d-patterns.php">patterns,</a>
        <a href="photography.php">photography,</a>
        <a href="d-political.php">political,</a>
        <a href="d-process.php">process,</a>
        <a href="w-product.php">product,</a>
        <a href="d-space.php">space,</a>
        <a href="d-typography.php">typography,</a>
        <a href="w-ux-wireframes.php">ux × wireframes,</a>
        <a href="d-visual-design.php">visual design,</a>
        <a href="w-websites.php">websites</a>
      </p>

      <p><strong>2021-OCT-06</strong> : Added images :
          <a href="inbox.php">inbox</a>
      </p>

      <p><strong>2021-JUN-20</strong> : Added images :
          <a href="inbox.php">inbox</a>
      </p>

      <p><strong>2021-FEB-28</strong> : Added images :
          <a href="inbox.php">inbox</a>
      </p>

      <p><strong>2020-FEB-07</strong> : Added images :
        <a href="w-agency-portfolio.php">agency × portfolio,</a>
        <a href="art-illustration.php">art × illustration,</a>
        <a href="d-brand-logos.php">brand × logos,</a>
        <a href="d-business-cards.php">business cards,</a>
        <a href="color.php">color,</a>
        <a href="copywriting.php">copywriting,</a>
        <a href="w-elements.php">elements,</a>
        <a href="ephemera.php">ephemera,</a>
        <a href="goods.php">goods,</a>
        <a href="d-process.php">process,</a>
        <a href="w-product.php">product,</a>
        <a href="quotes.php">quotes,</a>
        <a href="d-typography.php">typography,</a>
        <a href="w-ux-wireframes.php">ux × wireframes,</a>
        <a href="d-visual-design.php">visual design,</a>
        <a href="w-websites.php">websites</a>
      </p>

      <p><strong>2021-JAN-17</strong> : Added images :
        <a href="inbox.php">inbox</a>
      </p>

      <p><strong>2020-DEC-14</strong> : Added images :
        <a href="inbox.php">inbox,</a>
        <a href="w-ux-wireframes.php">ux × wireframes</a>
      </p>

      <p><strong>2020-DEC-01</strong> : Added images :
        <a href="inbox.php">inbox,</a>
        <a href="w-agency-portfolio.php">agency × portfolio</a>
        <a href="d-apps.php">apps,</a>
        <a href="art-illustration.php">art × illustration,</a>
        <a href="d-brand-logos.php">brand × logos,</a>
        <a href="d-business-cards.php">business cards,</a>
        <a href="goods.php">goods,</a>
        <a href="w-elements.php">website elements</a>
      </p>

      <p><strong>2020-NOV-25</strong> : Added images :
        <a href="inbox.php">inbox,</a>
        <a href="d-apps.php">apps,</a>
        <a href="art-illustration.php">art × illustration,</a>
        <a href="d-brand-logos.php">brand × logos,</a>
        <a href="d-business-cards.php">business cards,</a>
        <a href="color.php">color,</a>
        <a href="d-data-visualization.php">data visualization,</a>
        <a href="ephemera.php">ephemera,</a>
        <a href="w-elements.php">elements,</a>
        <a href="goods.php">goods,</a>
        <a href="d-patterns.php">patterns,</a>
        <a href="photography.php">photography,</a>
        <a href="d-process.php">process,</a>
        <a href="d-space.php">space,</a>
        <a href="d-typography.php">typography,</a>
        <a href="w-ux-wireframes.php">ux × wireframes,</a><br>
        <a href="d-visual-design.php">visual design</a>
        - Combined directories: design ― branding | design ― logos<br>
        - Combined directories: agency | portfolio<br>
        - Created directory: ux × wireframes
      </p>

      <p><strong>2020-JUL-18</strong> : Added images :
        <a href="art-illustration.php">art × illustration,</a>
        <a href="color.php">color</a>
      </p>

      <p><strong>2020-JUL-11</strong> : Added images :
        <a href="inbox.php">inbox,</a>
        <a href="art-illustration.php">art × illustration,</a>
        <a href="d-branding.php">branding,</a>
        <a href="d-business-cards.php">business cards,</a>
        <a href="goods.php">goods,</a>
        <a href="d-patterns.php">patterns,</a>
        <a href="d-political.php">political,</a>
        <a href="w-agency-portfolio.php">portfolios,</a>
        <a href="w-product.php">product websites,</a>
        <a href="d-typography.php">typography,</a>
        <a href="d-visual-design.php">visual design,</a>
        <a href="websites.php">websites,</a>
        <a href="w-elements.php">website elements</a>; removed directory : websites ― ecommerce (merged w/ websites)
      </p>

      <p><strong>2020-APR-28</strong> : Added images :
        <a href="art-illustration.php">art × illustration,</a>
        <a href="d-apps.php">apps,</a>
        <a href="color.php">color,</a>
        <a href="d-patterns.php">patterns,</a>
        <a href="w-agency-portfolio.php">portfolios,</a>
        <a href="d-space.php">space,</a>
        <a href="w-websites.php">websites</a> ; added directory : <a href="inbox.php">inbox</a>
      </p>

      <p><strong>2019-DEC-16</strong> : Added images :
        <a href="d-business-cards.php">business cards,</a>
        <a href="color.php">color,</a>
        <a href="d-data-visualization.php">data visualization,</a>
        <a href="w-agency-portfolio.php">portfolios,</a>
        <a href="w-product.php">product websites,</a>
        <a href="d-typography.php">typography,</a>
        <a href="d-visual-design.php">visual design,</a>
        <a href="websites.php">websites,</a>
        <a href="w-elements.php">website elements</a>
      </p>

      <p><strong>2019-DEC-6</strong> : Added images :
        <a href="art-illustration.php">art × illustration,</a>
        <a href="d-business-cards.php">business cards,</a>
        <a href="copywriting.php">copywriting,</a>
        <a href="ephemera.php">ephemera,</a>
        <a href="goods.php">goods,</a>
        <a href="d-logos.php">logos,</a>
        <a href="d-patterns.php">patterns,</a>
        <a href="photography.php">photography,</a>
        <a href="w-agency-portfolio.php">portfolios,</a>
        <a href="d-process.php">process,</a>
        <a href="d-space.php">space,</a>
        <a href="d-visual-design.php">visual design,</a>
        <a href="websites.php">websites,</a>
        <a href="w-elements.php">website elements</a>
      </p>

      <p><strong>2019-SEP-12</strong> : Added images :
        <a href="art-illustration.php">art × illustration,</a>
        <a href="d-branding.php">branding,</a>
        <a href="d-business-cards.php">business cards,</a>
        <a href="color.php">color,</a>
        <a href="d-patterns.php">patterns,</a>
        <a href="d-space.php">space,</a>
        <a href="d-typography.php">typography,</a>
        <a href="d-visual-design.php">visual design,</a>
        <a href="websites.php">websites,</a>
        <a href="w-elements.php">website elements</a>
      </p>

      <p><strong>2019-AUG-31</strong> : Added images :
        <a href="art-illustration.php">art × illustration,</a>
        <a href="color.php">color,</a>
        <a href="copywriting.php">copywriting,</a>
        <a href="ephemera.php">ephemera,</a>
        <a href="d-patterns.php">patterns,</a>
        <a href="photography.php">photography,</a>
        <a href="w-product.php">product websites,</a>
        <a href="d-space.php">space,</a>
        <a href="d-typography.php">typography,</a>
        <a href="d-visual-design.php">visual design,</a>
        <a href="websites.php">websites</a> ; renamed 'design ―― office' to 'design ―― space'.
      </p>

      <p><strong>2019-JUN-22</strong> : Added images :
          <a href="art-illustration.php">art × illustration,</a>
          <a href="d-branding.php">branding,</a>
          <a href="d-business-cards.php">business cards,</a>
          <a href="color.php">color,</a>
          <a href="ephemera.php">ephemera,</a>
          <a href="d-logos.php">logos,</a>
          <a href="d-patterns.php">patterns,</a>
          <a href="d-political.php">political,</a>
          <a href="w-agency-portfolio.php">portfolios,</a>
          <a href="d-process.php">process,</a>
          <a href="w-product.php">product websites,</a>
          <a href="d-space.php">space,</a>
          <a href="d-typography.php">typography,</a>
          <a href="d-visual-design.php">visual design,</a>
          <a href="websites.php">websites,</a>
          <a href="w-elements.php">website elements</a>
        </p>

        <p><strong>2019-APR-5</strong> : Added images :
          <a href="art-illustration.php">art × illustration,</a>
          <a href="d-branding.php">branding,</a>
          <a href="d-patterns.php">patterns,</a>
          <a href="w-agency-portfolio.php">portfolios</a>
          <a href="w-agency-portfolio.php">product websites,</a>
          <a href="d-visual-design.php">visual design,</a>
          <a href="websites.php">websites</a>
        </p>

        <p><strong>2019-MAR-13</strong> : Added images :
          <a href="d-apps.php">apps,</a>
          <a href="d-branding.php">branding,</a>
          <a href="copywriting.php">copywriting,</a>
          <a href="goods.php">goods,</a>
          <a href="d-patterns.php">patterns,</a>
          <a href="photography.php">photography,</a>
          <a href="w-agency-portfolio.php">portfolios,</a>
          <a href="d-visual-design.php">visual design,</a>
          <a href="websites.php">websites,</a>
          <a href="w-elements.php">website elements</a>
        </p>

        <p><strong>2019-FEB-21</strong> : Added images :
          <a href="art-illustration.php">art × illustration,</a>
          <a href="d-branding.php">branding,</a>
          <a href="copywriting.php">copywriting,</a>
          <a href="d-logos.php">logos,</a>
          <a href="photography.php">photography,</a>
          <a href="d-political.php">political,</a>
          <a href="w-agency-portfolio.php">portfolios,</a>
          <a href="w-product.php">product websites,</a>
          <a href="d-space.php">space,</a>
          <a href="d-typography.php">typography,</a>
          <a href="d-visual-design.php">visual design,</a>
          <a href="websites.php">websites,</a>
          <a href="w-elements.php">website elements</a>
        </p>

        <p><strong>2019-JAN-11</strong> : Added images :
          <a href="art-illustration.php">art × illustration,</a>
          <a href="d-branding.php">branding,</a>
          <a href="d-business-cards.php">business cards,</a>
          <a href="color.php">color,</a>
          <a href="copywriting.php">copywriting,</a>
          <a href="d-data-visualization.php">data visualization,</a>
          <a href="goods.php">goods,</a>
          <a href="d-logos.php">logos,</a>
          <a href="d-patterns.php">patterns,</a>
          <a href="photography.php">photography,</a>
          <a href="w-agency-portfolio.php">portfolios</a>
          <a href="d-space.php">space,</a>
          <a href="d-typography.php">typography,</a>
          <a href="d-visual-design.php">visual design,</a>
          <a href="websites.php">websites,</a>
          <a href="w-elements.php">website elements</a>
        </p>

        <p><strong>2018-DEC-18</strong> : Images are now displayed in random order. Updated homepage.</p>

        <p><strong>2018-DEC-15</strong> : Added images :
          <a href="art-illustration.php">art × illustration,</a>
          <a href="d-branding.php">branding,</a>
          <a href="d-business-cards.php">business cards,</a>
          <a href="copywriting.php">copywriting,</a>
          <a href="d-data-visualization.php">data visualization,</a>
          <a href="goods.php">goods,</a>
          <a href="d-patterns.php">patterns,</a>
          <a href="photography.php">photography,</a>
          <a href="w-agency-portfolio.php">portfolios,</a>
          <a href="d-space.php">space,</a>
          <a href="d-typography.php">typography,</a>
          <a href="d-visual-design.php">visual design,</a>
          <a href="websites.php">websites,</a>
          <a href="w-elements.php">website elements</a>
        </p>
        <p><strong>2018-NOV-22</strong> : Added images :
          <a href="art-illustration.php">art × illustration,</a>
          <a href="copywriting.php">copywriting,</a>
          <a href="d-business-cards.php">business cards,</a>
          <a href="d-data-visualization.php">data visualization,</a>
          <a href="goods.php">goods,</a>
          <a href="d-patterns.php">patterns,</a>
          <a href="d-typography.php">typography,</a>
          <a href="d-visual-design.php">visual design,</a>
          <a href="websites.php">websites,</a>
          <a href="w-elements.php">website elements</a>
        </p>
        <p><strong>2018-NOV-03</strong> : Added images :
          <a href="art-illustration.php">art × illustration,</a>
          <a href="d-branding.php">branding,</a>
          <a href="w-agency-portfolio.php">portfolios,</a>
          <a href="w-product.php">product websites</a>
          <a href="d-typography.php">typography,</a>
          <a href="d-visual-design.php">visual design,</a>
          <a href="websites.php">websites</a>
        </p>
        <p><strong>2018-OCT-14</strong> : Added images :
        <a href="d-apps.php">apps,</a>
        <a href="d-logos.php">logos,</a>
        <a href="w-product.php">product websites,</a>
        <a href="d-typography.php">typography,</a>
        <a href="d-visual-design.php">visual design</a>
        </p>
        <p><strong>2018-SEP-29</strong> : Added images :
          <a href="art-illustration.php">art × illustration,</a>
          <a href="d-business-cards.php">business cards,</a>
          <a href="d-political.php">political,</a>
          <a href="w-agency-portfolio.php">portfolios,</a>
          <a href="d-process.php">process,</a>
          <a href="w-product.php">product websites,</a>
          <a href="d-space.php">space,</a>
          <a href="d-visual-design.php">visual design</a>
        </p>
        <p><strong>2018-SEP-16</strong> : Added images :
          <a href="art-illustration.php">art × illustration,</a>
          <a href="color.php">color,</a>
          <a href="d-logos.php">logos,</a>
          <a href="w-agency-portfolio.php">portfolios,</a>
          <a href="d-process.php">process,</a>
          <a href="d-visual-design.php">visual design,</a>
          <a href="websites.php">websites</a>
        </p>
        <p><strong>2018-AUG-27</strong> : Added images :
          <a href="art-illustration.php">art × illustration,</a>
          <a href="w-agency-portfolio.php">portfolios,</a>
          <a href="d-typography.php">typography,</a>
          <a href="d-visual-design.php">visual design</a>
        </p>
        <p><strong>2018-AUG-28</strong> : Added images :
          <a href="art-illustration.php">art × illustration,</a>
          <a href="d-apps.php">apps,</a>
          <a href="d-business-cards.php">business cards,</a>
          <a href="d-logos.php">logos,</a>
          <a href="photography.php">photography,</a>
          <a href="w-agency-portfolio.php">portfolios,</a>
          <a href="d-visual-design.php">visual design,</a>
          <a href="websites.php">websites</a>
        </p>
        <p><strong>2018-AUG-21</strong> : Added images :
          <a href="d-branding.php">branding,</a>
          <a href="w-product.php">product websites</a>
        </p>
        <p><strong>2018-AUG-14</strong> : Added images :
        <a href="w-agency-portfolio.php">portfolios,</a>
        <a href="d-typography.php">typography,</a>
        <a href="d-visual-design.php">visual design</a>
        </p>
        <p><strong>2018-AUG-07</strong> : Added images :
          <a href="art-illustration.php">art × illustration,</a>
          <a href="d-business-cards.php">business cards,</a>
          <a href="color.php">color,</a>
          <a href="copywriting.php">copywriting,</a>
          <a href="photography.php">photography,</a>
          <a href="d-political.php">political,</a>
          <a href="w-agency-portfolio.php">portfolios,</a>
          <a href="d-typography.php">typography,</a>
          <a href="d-visual-design.php">visual design,</a>
          <a href="websites.php">websites</a>
        </p>
        <p><strong>2018-JUL-25</strong> : Launch</p>
      </div>
    <div class="column shim"></div>
  </article>

  <footer>
    <?php include('includes/footer.php'); ?>
  </footer>
  <?php include('includes/foot.php'); ?>
</body>
</html>
